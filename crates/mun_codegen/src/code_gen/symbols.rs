...
fn type_info_ir<D: IrDatabase>(
    ...
    module: &Module,
    types: &AbiTypes,
    type_map: &HashMap<Ty, TypeInfo>,
    type_values: &mut HashMap<Ty, PointerValue>,
    ty: Ty,
) -> PointerValue {
    if let Some(value) = type_values.get(&ty) {
        *value
    } else {
        let type_info = type_map.get(&ty).unwrap();
        ...
        let type_info_ir = match type_info.group {
            ...
            TypeGroup::StructTypes(s, t) => {
                let struct_info_ir =
                    gen_struct_info(db, target, module, types, type_map, type_values, s, t);
        ...
        type_values.insert(ty, type_info_ir);
        type_info_ir
    }
}
...
fn gen_signature_from_function<D: IrDatabase>(
    ...
    module: &Module,
    types: &AbiTypes,
    type_values: &HashMap<Ty, PointerValue>,
    ...
    let ret_type_ir = gen_signature_return_type(db, types, type_values, function);
    let (params_type_ir, num_params) =
        gen_signature_argument_types(db, module, types, type_values, function);
...
fn gen_signature_argument_types<D: IrDatabase>(
    ...
    module: &Module,
    types: &AbiTypes,
    type_map: &HashMap<Ty, PointerValue>,
    function: hir::Function,
) -> (PointerValue, usize) {
    ...
    gen_type_info_ptr_array(module, types, type_map, hir_types)
}
...
fn gen_type_info_array<D: IrDatabase>(
    ...
    if hir_types.peek().is_none() {
        ...
        let type_infos = hir_types
            .map(|ty| type_info_ir(db, target, module, types, type_map, type_values, ty))
...
fn gen_type_info_ptr_array(
    ...
    if hir_types.peek().is_none() {
        ...
        let type_infos = hir_types
            .map(|ty| *type_values.get(&ty).unwrap())
            .collect::<Vec<PointerValue>>();
...
fn gen_signature_return_type<D: IrDatabase>(
    ...
    if ret_type.is_empty() {
        ...
        *type_values.get(&ret_type).unwrap()
    }
}
...
fn gen_function_info_array<'a, D: IrDatabase>(
    ...
    let function_infos: Vec<StructValue> = functions
        .map(|(f, value)| {
            ...
            let signature = gen_signature_from_function(db, module, types, type_values, *f);
...
fn gen_struct_info<D: IrDatabase>(
    ...
    let (field_types, _) = gen_type_info_array(
        db,
        target,
        module,
        types,
...
fn gen_dispatch_table<D: IrDatabase>(
    ...
    let signatures: Vec<StructValue> = dispatch_table
        .entries()
        .iter()
        .map(|f| gen_signature_from_function(db, module, types, type_values, *f))
        .collect();
...
pub(super) fn gen_reflection_ir(
    ...
    let (types, num_types) = gen_type_info_array(
        db,
        target,
        module,
        &abi_types,
    ...
    let dispatch_table = gen_dispatch_table(db, module, &abi_types, &type_values, dispatch_table);
